...
public class CMSVideoFeedSingleFileImport : MonoBehaviour
{
    public string caption; // Set this to the caption of the video you want to download
    public bool alwaysDownload; // Set this to true if you want to always download the video
    ...
    void Start()
    {
        Debug.Log("Starting cms video feed import script...");
    ...
    IEnumerator GetDataFromAPI(string uri)
    {
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            foreach (var item in jsonData.data)
            {
                ...
                if (item.caption == caption && item.media_type == "Video")
                {
                    ...
                    if (fileExtension == ".mp4" || fileExtension == ".mov" || fileExtension == ".avi")
                    {
                        ...
                        if (alwaysDownload || !File.Exists(filePath) || File.GetLastWriteTime(filePath) < System.DateTime.Parse(item.updatedAt))
                        {
                ...
                {
                    Debug.Log("Item does not match caption or is not a video.");
                }
            }
        }
        else
