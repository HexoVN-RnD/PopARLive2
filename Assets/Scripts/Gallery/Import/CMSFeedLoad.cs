...
public class CMSFeedLoad : MonoBehaviour
{
    ...
    public Dictionary<int, string> gridTypes = new Dictionary<int, string>(); // the key is imageID, the value is grid type
    public Dictionary<int, Texture2D> mediaTextures = new Dictionary<int, Texture2D>(); // the key is imageID, the value is the image texture
    [System.NonSerialized]
    public List<int> imageIDs = new List<int>();

    private void Start()
    {
        // Load files from disk (skip waiting for event) if the import script is not enabled or not found
    ...
    {
        // Only subscribe to the event if the import script is enabled
        CMSFeedImport importScript = GetComponent<CMSFeedImport>();
        if (importScript != null && importScript.enabled)
        {
    ...
    IEnumerator DisplayImage(Data item)
    {
        ...
        // Change the name to item.id
        imageObject.name = item.id.ToString();
        ...
        if (!mediaTextures.ContainsKey(item.id))
        {
            mediaTextures.Add(item.id, texture);
        }
    ...
    IEnumerator DisplayVideo(Data item)
    {
        ...
        // Change the name to item.id
        videoThumbnailObject.name = item.id.ToString();
        ...
        if (!mediaTextures.ContainsKey(item.id))
        {
            mediaTextures.Add(item.id, thumbnailTexture);
        }
